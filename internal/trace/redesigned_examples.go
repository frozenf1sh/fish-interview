...
func redesignedFastSlowTrace() Trace {
	...
	frames := []Frame{
		...
		cycleListFrame(1, "fast=1 且 fast.Next=2 都非空，循环条件成立；先移动 slow。", "检查循环条件", map[string]string{"slow": "1", "fast": "1", "next": "2"}, cycleListNodes(map[string]string{"1": "current", "2": "dependency"}), cycleListLinks, map[string]string{"slow": "1", "fast": "1", "fast.Next": "2"}, nil),
		cycleListFrame(2, "slow 只沿一条 Next 边从 1 走到 2。", "slow 走一步", map[string]string{"slow": "2", "fast": "1"}, cycleListNodes(map[string]string{"1": "ready", "2": "current"}), cycleListLinks, map[string]string{"slow": "2", "fast": "1"}, nil),
		cycleListFrame(3, "fast 从 1 沿两条边走到 3；快指针的相对速度开始累积。", "fast 走两步", map[string]string{"slow": "2", "fast": "3"}, cycleListNodes(map[string]string{"1": "ready", "2": "dependency", "3": "current"}), cycleListLinks, map[string]string{"slow": "2", "fast": "3"}, nil),
		cycleListFrame(4, "2 与 3 不相等，继续下一轮；蓝色节点是比较所依赖的两个位置。", "第一次比较", map[string]string{"slow": "2", "fast": "3", "equal": "false"}, cycleListNodes(map[string]string{"2": "dependency", "3": "rejected"}), cycleListLinks, map[string]string{"slow": "2", "fast": "3"}, nil),
		cycleListFrame(2, "第二轮 slow 从 2 走到 3；它进入环后不会走向 nil。", "slow 再走一步", map[string]string{"slow": "3", "fast": "3"}, cycleListNodes(map[string]string{"3": "current"}), cycleListLinks, map[string]string{"slow": "3", "fast": "3"}, nil),
		cycleListFrame(3, "fast 从 3 经过 4 再回到 2；这一步正好展示 4→2 的回边。", "fast 沿环走两步", map[string]string{"slow": "3", "fast": "2", "path": "3→4→2"}, cycleListNodes(map[string]string{"2": "current", "3": "dependency", "4": "dependency"}), cycleListLinks, map[string]string{"slow": "3", "fast": "2"}, nil),
		cycleListFrame(4, "slow=3、fast=2 仍未相遇；相遇不是因为访问过，而是两个指针落在同一节点。", "第二次比较", map[string]string{"slow": "3", "fast": "2", "equal": "false"}, cycleListNodes(map[string]string{"2": "rejected", "3": "dependency"}), cycleListLinks, map[string]string{"slow": "3", "fast": "2"}, nil),
		cycleListFrame(2, "第三轮 slow 从 3 走到 4。", "slow 到 4", map[string]string{"slow": "4", "fast": "2"}, cycleListNodes(map[string]string{"2": "dependency", "4": "current"}), cycleListLinks, map[string]string{"slow": "4", "fast": "2"}, nil),
		cycleListFrame(3, "fast 从 2 经过 3 到 4；它与 slow 现在指向同一节点。", "fast 追上 slow", map[string]string{"slow": "4", "fast": "4"}, cycleListNodes(map[string]string{"4": "current"}), cycleListLinks, map[string]string{"slow": "4", "fast": "4"}, nil),
		cycleListFrame(4, "slow==fast，返回 true。完整环结构和每轮移动都可从图中复核。", "快慢指针：检测到环", map[string]string{"slow": "4", "fast": "4", "answer": "true"}, cycleListNodes(map[string]string{"4": "current"}), cycleListLinks, map[string]string{"slow": "4", "fast": "4"}, nil),
	}
